...
class SupabaseClient {
  ...
  async browseAgents(filters = {}) {
    if (this.demoMode) return this.getDemoBrowseAgents();
    ...
    if (category) {
      query = query.eq('user_skills.skill.category', category);
    }
    ...
    const { data, error } = await query;
    if (error) {
      console.warn('Browse agents query error:', error);
      // Fallback to demo data if no users yet
      return this.getDemoBrowseAgents();
    }
    
    // If no real users yet, show demo agents
