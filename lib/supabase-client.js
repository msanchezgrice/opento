  ...
  async browseAgents(filters = {}) {
    ...
    if (!this.demoMode) {
      try {
        ...
        // Try simpler query first - get users with profiles
        ...
        if (simpleError) {
          console.error('❌ Browse agents query error:', simpleError);
          console.log('Falling back to demo agents');
        } else {
          console.log(`📦 Raw query returned ${simpleData ? simpleData.length : 0} results`);
          
          if (simpleData && simpleData.length > 0) {
            // Enrich with skills separately
            const enrichedData = await Promise.all(simpleData.map(async (agent) => {
              // Fetch skills separately
              const { data: skillsData } = await this.client
                .from('user_skills')
                .select('skill_id, years_experience, skill:skills(id, name, category)')
                .eq('user_id', agent.id);
            ...
            console.log(`✅ Loaded ${filtered.length} agents from database (${enrichedData.length} total, ${filtered.length} after filters)`);
            return filtered;
          }
        }
      } catch (err) {
