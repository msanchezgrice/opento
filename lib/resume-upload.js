...
window.handleResumeUpload = async function(event) {
  ...
  // Validate file type (PDF temporarily disabled on Vercel)
...
function prefillFromResume(data) {
  ...
  if (data.skills && Array.isArray(data.skills) && data.skills.length > 0) {
    ...
    setTimeout(() => {
      // Use the global addSkill function from skills-selector.js
      if (typeof window.addSkill === 'function') {
        console.log('Using window.addSkill function');
        ...
        console.warn('window.addSkill not available, trying fallback');
          ...
          data.skills.forEach((skill, index) => {
            setTimeout(() => {
              ...
              setTimeout(() => {
                ...
                if (firstItem) {
                  firstItem.click();
                  console.log(`✓ Added skill via fallback: ${skill}`);
                }
              }, 100);
            }, index * 300);
          });
    ...
    }, 800); // Wait longer for skills selector to be ready
  }
