...
(function() {
  ...
  async function testSignIn(handle) {
    try {
      ...
      console.log(`Attempting to sign in as: ${handle}`);
      console.log(`API URL: ${API_BASE}/agents/${handle}`);
      ...
      if (!response.ok) {
        ...
        throw new Error(`User not found (${response.status}). Make sure the handle exists in the database.`);
      }
      ...
      setSession({
        ...
        name: userData.displayName || userData.display_name
      });
      
      console.log(`✓ Test sign-in successful: ${userData.displayName || userData.display_name} (@${userData.handle})`);
      return userData;
