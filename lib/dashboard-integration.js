...
(function() {
  ...
  function populateDashboard(data) {
    ...
    // Header - show photo if available, otherwise initials
    const dashAvatar = document.getElementById('dashAvatar');
    if (dashAvatar) {
      if (data.avatar_url) {
        dashAvatar.innerHTML = `<img src="${data.avatar_url}" alt="${data.displayName}" style="width: 100%; height: 100%; object-fit: cover; border-radius: 50%;">`;
      } else {
        dashAvatar.textContent = data.avatarInitials || '—';
      }
    }
    
    // Show "Preview My Agent" button if user has a handle
    const previewBtn = document.getElementById('previewAgentBtn');
    if (previewBtn && data.handle) {
    ...
    const viewProfileLink = document.getElementById('viewPublicProfile');
    if (viewProfileLink && data.handle) {
      viewProfileLink.href = `/handle.html?u=${data.handle}`;
    }
    ...
    // Populate Profile form - Basic info
    if (document.getElementById('profileName')) document.getElementById('profileName').value = data.displayName || '';
    if (document.getElementById('profileProfessionalTitle')) document.getElementById('profileProfessionalTitle').value = data.professional_title || '';
    ...
    if (document.getElementById('profileSeniorityLevel')) document.getElementById('profileSeniorityLevel').value = data.seniority_level || '';
    if (document.getElementById('profileCurrentCompany')) document.getElementById('profileCurrentCompany').value = data.current_company || '';
    if (document.getElementById('profileYearsInRole')) document.getElementById('profileYearsInRole').value = data.years_in_role || '';
    if (document.getElementById('profileIndustries')) document.getElementById('profileIndustries').value = (data.industries || []).join(', ');
    
    // Populate best_at fields
    const bestAtInputs = document.querySelectorAll('.profile-best-at-input');
    const bestAtData = data.best_at || [];
    bestAtInputs.forEach((input, idx) => {
      input.value = bestAtData[idx] || '';
    });
    
    // Populate experience_highlights fields
    const highlightInputs = document.querySelectorAll('.profile-highlight-input');
    const highlightsData = data.experience_highlights || [];
    highlightInputs.forEach((input, idx) => {
      input.value = highlightsData[idx] || '';
    });
    ...
    // Populate Agent Settings summary
    populateAgentSettingsSummary(data);
  }
  
  function populateAgentSettingsSummary(data) {
    const settings = data.settings || {};
    
    // Rates
    const consultFloor = settings.consult_floor_30m || 75;
    const asyncFloor = settings.async_floor_5m || 12;
    setText('#summaryConsultFloor', `$${consultFloor}`);
    setText('#summaryAsyncFloor', `$${asyncFloor}`);
    
    // Availability
    const weeklyHours = settings.weekly_hours || 6;
    const availWindow = settings.availability_window || 'Mon–Thu 11a–4p CT';
    setText('#summaryWeeklyHours', weeklyHours);
    setText('#summaryAvailabilityWindow', availWindow);
    
    // Categories
    const categories = settings.categories || [];
    const categoriesDiv = document.getElementById('summaryCategories');
    if (categoriesDiv) {
      if (categories.length > 0) {
        categoriesDiv.innerHTML = categories.map(cat => `<div class="chip">${cat}</div>`).join('');
      } else {
        categoriesDiv.innerHTML = '<div class="small muted">No categories selected</div>';
      }
    }
    
    // Privacy
    const privacyDiv = document.getElementById('summaryPrivacy');
    if (privacyDiv) {
      const anonFirst = settings.anonymous_first !== false;
      const consentReminders = settings.consent_reminders !== false;
      privacyDiv.innerHTML = `
        <div style="margin-bottom: 4px;">${anonFirst ? '✓' : '✗'} Anonymous first</div>
        <div>${consentReminders ? '✓' : '✗'} Consent reminders</div>
      `;
    }
  }
  ...
  async function saveProfile() {
    try {
      ...
      // Collect best_at entries
      const bestAtInputs = Array.from(document.querySelectorAll('.profile-best-at-input'));
      const bestAt = bestAtInputs.map(input => input.value.trim()).filter(v => v.length > 0);
      
      // Collect experience_highlights entries
      const highlightInputs = Array.from(document.querySelectorAll('.profile-highlight-input'));
      const highlights = highlightInputs.map(input => input.value.trim()).filter(v => v.length > 0);
      
      // Parse industries
      const industriesValue = document.getElementById('profileIndustries')?.value || '';
      const industries = industriesValue.split(',').map(i => i.trim()).filter(i => i.length > 0);

      const profileData = {
        userId: user.id,
        displayName: document.getElementById('profileName').value.trim(),
        professionalTitle: document.getElementById('profileProfessionalTitle')?.value.trim() || null,
        ...
        // Enhanced fields
        seniorityLevel: document.getElementById('profileSeniorityLevel')?.value || null,
        currentCompany: document.getElementById('profileCurrentCompany')?.value.trim() || null,
        yearsInRole: document.getElementById('profileYearsInRole')?.value ? parseInt(document.getElementById('profileYearsInRole').value) : null,
