...
(function() {
  ...
  window.addEventListener('DOMContentLoaded', async () => {
    try {
      ...
      window.dispatchEvent(new CustomEvent('agentDataLoaded', { detail: agentData }));
      ...
      window.dispatchEvent(new CustomEvent('agentDataError', { detail: error }));
    }
  });
  ...
  function populateAgentPage(agent) {
    ...
    // Use user-defined professional_title instead of auto-generated role
    setText('#handleRole', agent.professional_title || agent.role || 'Professional');
    // Use user-written bio instead of auto-generated summary
    setText('#handleSummary', agent.bio || agent.summary || 'Building their presence on Opento');
    ...
    if (avatar) {
      if (agent.avatar_url) {
        avatar.innerHTML = `<img src="${agent.avatar_url}" alt="${agent.displayName}" style="width: 100%; height: 100%; object-fit: cover; border-radius: 50%;">`;
      } else {
        avatar.textContent = agent.avatar || agent.avatar_initials || '—';
      }
    }
    ...
    // Experience section (enhanced fields)
    const experienceCard = qs('#experienceCard');
    const seniorityChip = qs('#seniorityChip');
    const currentRoleInfo = qs('#currentRoleInfo');
    const industriesSection = qs('#industriesSection');
    
    // Show experience card if any enhanced fields are present
    const hasExperienceData = agent.seniority_level || agent.current_company || agent.years_in_role || (agent.industries && agent.industries.length > 0);
    if (experienceCard) {
      experienceCard.style.display = hasExperienceData ? '' : 'none';
    }
    
    if (seniorityChip && agent.seniority_level) {
      seniorityChip.textContent = agent.seniority_level;
      seniorityChip.style.display = '';
    } else if (seniorityChip) {
      seniorityChip.style.display = 'none';
    }
    
    if (currentRoleInfo) {
      if (agent.current_company && agent.years_in_role) {
        const years = agent.years_in_role;
        const yearText = years === 0 ? '<1 year' : years === 6 ? '6+ years' : `${years} ${years === 1 ? 'year' : 'years'}`;
        currentRoleInfo.innerHTML = `<span class="small">at <strong>${agent.current_company}</strong> · ${yearText}</span>`;
      } else if (agent.current_company) {
        currentRoleInfo.innerHTML = `<span class="small">at <strong>${agent.current_company}</strong></span>`;
      } else if (agent.years_in_role) {
        const years = agent.years_in_role;
        const yearText = years === 0 ? '<1 year' : years === 6 ? '6+ years' : `${years} ${years === 1 ? 'year' : 'years'}`;
        currentRoleInfo.innerHTML = `<span class="small">${yearText} in current role</span>`;
      }
    }
    
    if (industriesSection && agent.industries && agent.industries.length > 0) {
      industriesSection.innerHTML = `
        <div class="small muted" style="margin-bottom: 8px;">Industries:</div>
        <div class="chips">
          ${agent.industries.map(industry => `<div class="chip">${industry}</div>`).join('')}
        </div>
      `;
    ...
    // Skills display
    const skillsCard = qs('#skillsCard');
    const skillsSection = qs('#skillsSection');
    if (skillsSection && agent.skills && agent.skills.length > 0) {
      skillsSection.innerHTML = `
        <div class="chips">
          ${agent.skills.map(skill => `<div class="chip">${skill}</div>`).join('')}
        </div>
      `;
      if (skillsCard) skillsCard.style.display = '';
    } else if (skillsCard) {
      skillsCard.style.display = 'none';
    }
    
    // Best At section (user-written strengths)
    const bestAtList = qs('#bestAtList');
    if (bestAtList) {
      if (agent.best_at && agent.best_at.length > 0) {
        bestAtList.innerHTML = agent.best_at
          .map(item => `<li>${item}</li>`)
          .join('');
      } else if (agent.focusAreas && agent.focusAreas.length > 0) {
        // Fallback to auto-generated focus areas if no user-written content
        bestAtList.innerHTML = agent.focusAreas
          .map(item => `<li>${item}</li>`)
          .join('');
      } else {
        bestAtList.innerHTML = '<li class="muted">Building their profile</li>';
      }
    }
    ...
    // Recent wins/Experience highlights (user-written achievements)
    const winList = qs('#winList');
    if (winList) {
      if (agent.experience_highlights && agent.experience_highlights.length > 0) {
        // Use user-written highlights
        winList.innerHTML = agent.experience_highlights
          .map(item => `<li>${item}</li>`)
          .join('');
      } else if (agent.recentWins && agent.recentWins.length > 0) {
        // Fallback to auto-generated wins
        winList.innerHTML = agent.recentWins
          .map(item => `<li>${item}</li>`)
          .join('');
      } else {
        winList.innerHTML = '<li class="muted">Building track record</li>';
      }
    }
    ...
    // Initialize chat now that agent data is loaded
