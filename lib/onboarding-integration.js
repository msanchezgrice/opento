  ...
  window.startAgent = async function(path) {
    try {
      ...
      if (!userId) {
        ...
        // Generate display name from form (use LinkedIn name if available)
        const defaultName = linkedInName || 'Agent User';
        const displayName = prompt('What name should we use for your agent profile?', defaultName);
