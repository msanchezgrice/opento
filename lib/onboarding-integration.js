...
(function() {
  ...
  window.startAgent = async function(path) {
    try {
      ...
      const skillsText = qs('#skills')?.value || '';
      ...
      if (!userId) {
        ...
        // Generate display name from form (we don't have a name field, use placeholder)
        const displayName = prompt('What name should we use for your agent profile?', 'Agent User');
      ...
      // Call original function for UI updates or redirect
      if (originalStartAgent) {
        originalStartAgent(path);
      } else {
        toast('🎉 Agent created! Redirecting to your profile...');
        // Redirect to their agent page instead of inbox
        setTimeout(() => {
          const userHandle = localStorage.getItem('opento_user_handle');
          if (userHandle) {
            window.location.href = `handle.html?u=${userHandle}`;
          } else {
            window.location.href = 'inbox.html?tab=micro';
          }
        }, 1200);
      }
